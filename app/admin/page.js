      ...
      <div style={{ marginLeft: "260px", flex: 1, padding: "40px" }}>
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "25px", marginBottom: "40px" }}>
              <StatCard title="Total Sales" value={`₹${orders.reduce((a,b)=>a+(Number(b.price)||0),0)}`} icon={<TrendingUp color="#16a34a"/>} color="#dcfce7" />
            ...
            <div style={cardStyle}>
              ...
              <OrderTable orders={orders.slice(0,5)} updateStatus={updateOrderStatus} />
            </div>
          </>
        )}
        ...
        {activeTab === 'orders' && (
          <>
            ...
            <div style={cardStyle}>
              <OrderTable orders={orders} updateStatus={updateOrderStatus} />
            </div>
          </>
        )}
      </div>
      ...
      <tbody>
        {orders.map(o => (
          <tr key={o.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
            <td style={tdStyle}>{o.orderDate?.split(',')[0]}</td>
            <td style={tdStyle}><b>{o.customerName}</b></td>
            <td style={tdStyle}>{o.productName}</td>
            <td style={{ ...tdStyle, color: "#16a34a", fontWeight: "bold" }}>₹{o.price}</td>
            <td style={tdStyle}>
              <select 
                value={o.status} 
