...
export default function AdminDashboard() {
  ...
  // --- TRACKING LOGIC ---
  const saveTrackingId = (orderId) => {
    const tId = tempTrackingIds[orderId];
    if (!tId) return alert("Pehle Tracking ID likhein! 📦");
    update(ref(db, `orders/${orderId}`), { trackingId: tId });
    alert("Tracking ID Save Ho Gayi! ✅");
  };

  const updateOrderStatus = (orderId, newStatus, currentTracking) => {
    // Delivered check: Bina Tracking ID ke deliver nahi hoga
              ...
              <tr key={o.id} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td style={tdStyle}>
                  <div style={{display:"flex", flexDirection:"column", gap:"5px"}}>
                    ...
                    />
                    <button 
                      onClick={() => saveTrackingId(o.id)}
                      style={{background:"#1b5e20", color:"white", border:"none", fontSize:"10px", borderRadius:"4px", cursor:"pointer", padding:"2px"}}
                    >Save ID</button>
                  </div>
                </td>
